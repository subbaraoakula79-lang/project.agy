...
interface DriverRideHistoryItem {
  id: string;
  status: string;
  requestedAt: string;
...
export default function DriverHistoryScreen() {
  const [rides, setRides] = useState<DriverRideHistoryItem[]>([
    {
      ...
      location: {
        pickupAddress: 'Kakinada Beach Road',
        dropAddress: 'JNTU Kakinada Campus',
      },
      rider: {
        firstName: 'Anil',
        lastName: 'Kumar',
    ...
    },
    {
      ...
      cancelledAt: new Date(Date.now() - 172500000).toISOString(),
      cancellationReason: 'Rider plans changed',
      estimatedFare: 108,
      paymentMethod: 'UPI',
      location: {
        pickupAddress: 'Sarpavaram Junction',
        dropAddress: 'Kakinada Bus Stand',
      },
      rider: {
        firstName: 'Venkatesh',
        lastName: 'P',
  ...
  const [selectedRide, setSelectedRide] = useState<DriverRideHistoryItem | null>(null);
  ...
  const handleOpenRating = (ride: DriverRideHistoryItem) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>🚖 Captain Trip History</Text>
      <Text style={styles.subtitle}>Your assigned trips in Kakinada, AP</Text>
          ...
          return (
            <View style={styles.rideCard}>
              <View style={styles.rideCardHeader}>
                <Text style={styles.fareText}>Earnings: ₹{item.actualFare || item.estimatedFare}</Text>
                <View
                  style={[
                    styles.statusBadge,
              ...
              {item.rider && (
                <Text style={styles.riderText}>
                  👤 Passenger: {item.rider.firstName} {item.rider.lastName} (⭐ {item.rider.averageRating || 'New'})
                </Text>
              )}

              {/* Rating Section for Driver */}
              {isCompleted && (
                <View style={styles.ratingSection}>
                  {item.myRating ? (
                    <View style={styles.submittedRatingBox}>
                      <Text style={styles.submittedRatingText}>
                        ✅ Rated Passenger: {'⭐'.repeat(item.myRating.rating)} ({item.myRating.rating}/5)
                      </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSub}>
              Rate your experience with {selectedRide?.rider?.firstName || 'Passenger'}
            </Text>
            ...
            <TextInput
              ...
              placeholder="Write an optional feedback for passenger..."
